...
if($_POST)
    {
        ...
        $oldpassquery= mysqli_query($con,"select Supplier_pass from supplierregistration where Supplier_id='{$_SESSION['session_id']}'")  or die(mysqli_error($con));
        $oldpassfetch= mysqli_fetch_array($oldpassquery);
        if( $oldpassfetch['Supplier_pass']== $opass)
        {
            if($npass == $cpass)
            {
                if($opass == $npass)
                ...
                {
                    $updatequery= mysqli_query($con, "Update supplierregistration set Supplier_pass='{$npass}' where Supplier_id='{$_SESSION['session_id']}' ") or die(mysqli_error($con));
                    if($updatequery)
                    {
