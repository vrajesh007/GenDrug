...
$response = array();
if (isset($_POST['opass']) && !empty($_POST['opass']) && isset($_POST['npass']) && !empty($_POST['npass']) && isset($_POST['cpass']) && !empty($_POST['cpass']) && isset($_POST['U_id']) && !empty($_POST['U_id'])) {
    ...
    $check_user = mysqli_query($con, "select Password from userregistration where U_id = '{$U_id}'") or die(mysqli_error($con));
    $count = mysqli_num_rows($check_user);

    if ($count > 0) {

        $check_old_password = mysqli_query($con, "select * from userregistration where U_id = '{$U_id}' and Password = '{$opass}'") or die(mysqli_error($con));
        ...
        if ($oldpassfromdb['Password'] == $opass) {
            ...
            if ($npass == $cpass) {
                ...
                if ($count > 0) {
                    ...
                    $updatequery = mysqli_query($con, "update userregistration set Password='{$npass}' where U_id = '{$U_id}'") or die(mysqli_error($con));
